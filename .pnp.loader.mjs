...
async function copyFileViaIndex(
  ...
  const sourceHash = await sourceFs.checksumFilePromise(
    source,
    { algorithm: `sha1` }
  );
...
function resolvePackageTargetString(
  ...
  if (
    RegExpPrototypeExec(invalidSegmentRegEx, subpath) !==
    null
  ) {
...
async function resolve$1(
  ...
  const url = tryParseURL(
    specifier,
    isRelativeRegexp.test(specifier)
      ? context.parentURL
      : void 0
  );
  if (url) {
  ...
  const issuer =
    parentURL &&
    tryParseURL(parentURL)?.protocol === `file:`
      ? fileURLToPath(parentURL)
      : process.cwd();
  const pnpapi =
    findPnpApi(issuer) ??
    (url ? findPnpApi(specifier) : null);
  if (!pnpapi)
    return nextResolve(
      originalSpecifier,
      context,
