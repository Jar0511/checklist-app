...
import { FolderAtom, FolderErrAtom, FolderLoading } from "./store";
import { getFolderList } from "../api";
import type { ErrorResponse } from "react-router-dom";

export const fetchFolder = atom(null, async (get, set, path?: string, firstFetch?: boolean) => {
  ...
  try {
    ...
    set(FolderAtom, {
      folder: [...(firstFetch ? [] : folder), ...list.map((folder) => folder.name)],
      page: page + 1,
      more: list.length >= 50
    });
