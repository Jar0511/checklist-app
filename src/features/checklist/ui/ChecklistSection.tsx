...
const AddCheckListForm = ({
	...
	const filteredChecklist = useMemo(() => {
		/** 중복되는 항목을 입력했는지 여부 */
		const IsDuplicateName = checklist.some(
			({ title, checked }) =>
				title == inputText.trim() && !checked
		);
		/** 존재하는 항목을 입력했는지 여부 */
		const IsExistName = checklist.some(
			({ title, checked }) =>
				title == inputText.trim() && checked
		);
		return [
	...
	const deboundedOnChange = useDebounce(
		handleChangeEvent,
		500
	);
		...
		if (e.key == "ArrowUp") {
			...
			return setSelectIndex((prev) =>
				prev == undefined
					? 0
					: filteredChecklist.length > 0
						? prev + 1 >= filteredChecklist.length
							? 0
							: prev + 1
						: undefined
			);
	...
	useDismissClick(".new_check_container", () =>
		setFocus(false)
	);
...
const CheckItem = ({
	...
	return (
		...
		>
			<p className="flex-1 cursor-default">{title}</p>
			<BasicButton
				onClick={async () => {
...
export const ChecklistSection = () => {
	const { checklist } = useLoaderData() as {
		checklist: Promise<Tables<"checklist">[]>;
	};
