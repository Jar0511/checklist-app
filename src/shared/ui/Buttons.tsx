...
const makeFilledClass = ({
	className,
	btncolor,
	dim,
}: { className?: string } & CommonButtonType) => {
	if (className?.includes("bg-")) return "";
	if (btncolor == "secondary" && dim) {
		consoleWarn({
			title: "secondary 컬러 버튼",
			desc: `secondary는 dim 여부가 적용되지 않습니다`,
		});
	}
	...
	switch (btncolor) {
		case "primary":
			...
			break;
		case "secondary":
			bgClass = `bg-red-800/10 dark:bg-red-200/20 hover:bg-red-800/20 dark:hover:bg-red-200/30 ${darkText}`;
			break;
		case "tertiary":
			bgClass = dim
...
export const BasicButton = forwardRef<
	...
	ButtonHTMLAttributes<HTMLButtonElement> &
		Pick<CommonButtonType, "modal">
>(
	(
		{ modal, type, title, children, className, ...rest },
		ref
	) => {
	...
	(
		...
		return (
			<BasicButton
				...
				ref={ref}
				title={title}
				className={`${className?.includes("font-") ? "" : "font-bold"} ${
...
export const OutlinedButton = forwardRef<
	HTMLButtonElement,
	ButtonHTMLAttributes<HTMLButtonElement> &
		Omit<CommonButtonType, "dim" | "btncolor"> & {
			primary?: boolean;
		}
>(
	(
				...
				className={`${
					...
					className?.includes("font-")
						...
						: size == "lg"
							? `font-bold`
							: `font-semibold`
				} ${size == "lg" ? "border-2" : "border"} border-solid ${
					// border
					primary
						? "border-grapefruit-500 dark:border-grapefruit-600 text-grapefruit-500 dark:text-grapefruit-600 hover:bg-grapefruit-400/20"
